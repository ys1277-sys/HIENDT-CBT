   ...
   ══════════════════════════════════════════ */
{
  ...
  const BIG = [
    ["1,369", "문항", "종이 시험지 49벌을 다 옮기고,\n40문항 시험에 60문항을 채웠습니다"],
    ["4,200조", "가지 시험지", "60문항에서 40개를 뽑는 가짓수.\n앞사람 시험지를 봐도 소용없습니다"],
    ["44", "개 조항", "문항 수 · 합격선 · 면제 · 재시험 ·\n만료 · 기록 보존까지 다 옮겼습니다"],
    ["15", "가지 자동 검사", "채점 · 보기 섞기 · 날짜 · 규정 대조 …\n고칠 때마다 명령 한 줄로 다 돕니다"],
    ["20만", "줄", "3주 동안 고친 양입니다.\n한 줄씩 직접 짰습니다"],
    ["0", "원", "서버를 사지 않았습니다.\n달마다 나가는 돈이 없습니다"],
  ];
...
{
  const s = p.addSlide();
  banner(s, "만든 방법 ②-2  그 44개가 무엇인가");
  sub(s, "「조항을 옮겼다」로 지나가면 뭘 옮겼는지 모릅니다. 갈래로 묶으면 이렇습니다");

  /*
   * 세어 온 값이다 — E02·E03 이 E01 을 끌어다 쓴 조항을 모두 모아
   * 갈래로 묶었다. 여덟 갈래 44개.
   */
  const G = [
    ["누가 책임지나", "8",
     "문제 승인 · 시험 운영과 채점 · 시험지 보관 · 본인이나 부하직원 시험은 맡지 않기"],
    ["응시할 수 있는 사람", "2",
     "교육과 경력 요건 · 시력검사는 받은 날로부터 1년"],
    ["시험을 어떻게 짜나", "4",
     "일반 · 전문 · 실기로 나눔 · Level Ⅲ 기초 55, 종목 65 · 종목마다 몇 문제를 낼지"],
    ["면제", "5",
     "ASNT · ISO 9712 소지자는 몇 과목 면제 · 대신 치르는 시험은 80%"],
    ["참고자료와 채점", "4",
     "시험 중 무엇을 볼 수 있나 · 종합은 단순 평균 · 개별 70% 종합 80% · 외부 위탁은 80%"],
    ["재시험", "1",
     "떨어지면 30일 지나서 · 추가 교육 증거가 있으면 앞당길 수 있음"],
    ["자격증과 기록", "11",
     "발행 조건 · 훈련생에게는 발행 안 함 · 퇴사 뒤 5년 보존 · 자격종료와 복권"],
    ["재자격과 능력 평가", "9",
     "기술수행 능력 평가 · 6개월 이상 손 놓으면 자격 정지 · 다시 받는 주기 Ⅰ·Ⅱ 3년, Ⅲ 5년"],
  ];

  const y0 = 1.62, rh = 0.63;
  G.forEach((g, i) => {
    const y = y0 + i * rh;
    s.addShape(p.ShapeType.rect, {
      x: 0.62, y, w: 12.55, h: 0.56,
      fill: { color: i % 2 ? "FFFFFF" : "F9F9F9" }, line: { color: LINE, width: 1 },
    });
    s.addShape(p.ShapeType.rect, {
      x: 0.62, y, w: 0.72, h: 0.56,
      fill: { color: "F2F8FD" }, line: { color: LINE, width: 1 },
    });
    s.addText(g[1], {
      x: 0.62, y, w: 0.72, h: 0.56, isTextBox: true, margin: 0,
      fontFace: F, fontSize: 17, bold: true, color: RED,
      align: "center", valign: "middle",
    });
    s.addText(g[0], {
      x: 1.5, y, w: 3.1, h: 0.56, isTextBox: true, margin: 0,
      fontFace: F, fontSize: 14.5, bold: true, color: INK, valign: "middle",
    });
    s.addText(g[2], {
      x: 4.7, y, w: 8.3, h: 0.56, isTextBox: true, margin: 0,
      fontFace: F, fontSize: 11.5, color: MUT, valign: "middle",
    });
  });

  s.addShape(p.ShapeType.rect, {
    x: 0.62, y: 6.72, w: 12.55, h: 0.56,
    fill: { color: "F2F8FD" }, line: { color: BLU, width: 1 },
  });
  s.addText("모두 44개 조항 · 109군데에서 인용", {
    x: 0.9, y: 6.72, w: 4.2, h: 0.56, isTextBox: true, margin: 0,
    fontFace: F, fontSize: 14, bold: true, color: BLU, valign: "middle",
  });
  s.addText("조항마다 「이건 E01 어디에서 온 것이다」를 달아 두어, 규정이 바뀌면 그 자리만 찾으면 됩니다.", {
    x: 5.2, y: 6.72, w: 7.7, h: 0.56, isTextBox: true, margin: 0,
    fontFace: F, fontSize: 12.5, color: INK, valign: "middle",
  });

  page(s);
  s.addNotes("이 44개를 한 줄씩 원문과 맞대어 봤습니다. 그러다 어긋난 곳 셋을 찾았습니다 — 뒤에서 보여 드립니다.");
}
